...
// These are the official Google Calendar colors from the API
// Two palettes: calendar colors (1-24) and event colors (1-11)
...
// Get all event colors as an array for UI rendering
function getColorOptions(colorType = 'event') {
    const palette = GOOGLE_CALENDAR_COLORS[colorType] || GOOGLE_CALENDAR_COLORS.event;
    return Object.entries(palette).map(([colorId, colorData]) => ({
        id: colorId,
        hex: colorData.background
    }));
}

// Get color by hex value (searches both palettes)
function getColorByHex(hexColor) {
    const normalizedInput = hexColor.toLowerCase();
    
    // Search in event colors first
    for (const [colorId, colorData] of Object.entries(GOOGLE_CALENDAR_COLORS.event)) {
        if (colorData.background.toLowerCase() === normalizedInput) {
            return { id: colorId, hex: colorData.background, type: 'event' };
        }
    }
    
    // Then search in calendar colors
    for (const [colorId, colorData] of Object.entries(GOOGLE_CALENDAR_COLORS.calendar)) {
        if (colorData.background.toLowerCase() === normalizedInput) {
            return { id: colorId, hex: colorData.background, type: 'calendar' };
        }
    }
    
    // Return default if not found
    return { id: '1', hex: '#a4bdfc', type: 'event' };
}
